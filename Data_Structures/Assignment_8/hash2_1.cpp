...
/*
          if(overflowTable[a]->presentStatus == 0){   //if the corresponding overflow slot is empty
            ...
            if(primaryTable[a]->index == -1){
              ...
              cout<<"HI"<<j<<endl;
              if(isEmpty == 1){
                overflowTable[j]->value = x;
                cout<<"HI"<<endl;
                overflowTable[j]->presentStatus = 1;
                cout<<"HI1"<<endl;
                primaryTable[a]->index = j;
              }
    ...
    void del(int x){                                            //function to delete a value from a node
        ...
        if(primaryTable[a]->value == x){
            ...
            else if(overflowTable[primaryTable[a]->index]->index == -1){
                primaryTable[a]->value = overflowTable[primaryTable[a]->index]->value;
                overflowTable[primaryTable[a]->index]->value = -1;
                overflowTable[primaryTable[a]->index]->presentStatus = 0;
                primaryTable[a]->index = -1;

            }
            else{
              primaryTable[a]->value = overflowTable[primaryTable[a]->index]->value;//**
              a =primaryTable[a]->index;
